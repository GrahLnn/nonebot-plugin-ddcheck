...
async def get_tweets(interval: int = 2):
    ...
    try:
        ...
        if not tweet_elements:
            logger.warning("未找到任何推文，可能是页面加载失败")
        ...
        for tweet in tweet_elements:
            ...
            tweet_data["author_name"] = (
                author_details[0] if len(author_details) > 0 else ""
            )
            ...
            if tweet_data["author_handle"] != "@MariaMari0nette":
                continue
            # 获取推文链接
            tweet_link = tweet.ele('xpath:.//a[contains(@href, "/status/")]').attr(
                "href"
            )
            ...
            image_urls = [
                img.attr("src").replace(
                    "?format=jpg&name=small", "?format=jpg&name=large"
                )
